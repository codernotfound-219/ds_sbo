...
use super::utils::{create_end, create_after, create_before, employ_insert_action};
    ...
    loop {
        ...
        if let Some(batch_index) = locate_eligible_batch(&schedule, current.due_date) {
                ...
                match result {
                    ...
                    Decision::InsertAtPosition(_, actions) => {employ_insert_action(&mut schedule, current, &actions);},
                    Decision::InsertBefore(_, actions) => {employ_insert_action(&mut schedule, current, &actions);},
                    Decision::InsertAfter(_, actions) => {employ_insert_action(&mut schedule, current, &actions);},
                }
            } else {
                ...
                match result {
                    ...
                    EndDecision::InsertAtLast(_, actions) => {employ_insert_action(&mut schedule, current, &actions);},
                }
            }
    }
