...
//         let mut set_m: Vec<ActiveLog> = Vec::new();
...
//                     set_m.push(ActiveLog::new(i32::MIN, Decision::NotPossible));
//                 }
//                 Ordering::Greater => {
//                     // Calculate deviation for inserting job in current batch (after removing lp_job)
//                     let (current_deviation, completion) =
//                         compute_current_deviation(&job_list, job, current_prev_completion); // prev_completion remains 0
//                     // set_m.push(ActiveLog::new(
//                     //     current_deviation,
//                     //     Decision::InsertIn {
//                     //         batch_index: index,
//                     //         job_code: job.code,
...
//                     let all_recursive_results = get_active_logs_for_displaced_job(
//                         index + 1,
//                         schedule,
//                         &lp_job,
//                         completion,
...
//                     for possibility in all_recursive_results {
//                         let aggregate: i32 = possibility
//                             .iter()
//                             .filter(|log| log.deviation < 0)
//                             .map(|log| log.deviation)
//                             .sum();
//                         let min_deviation = possibility.iter().map(|log| log.deviation).min().unwrap();
...
// // New helper function to get individual ActiveLogs from displaced job placement
// fn get_active_logs_for_displaced_job(
//     batch_index: usize,
//     schedule: &BatchSchedule,
//     job: &Job,
//     prev_completion: u32,
//     updated_completion_at_index: Option<(usize, u32)>,
// ) -> Vec<ActiveLog> {
//     let mut all_possibilities: Vec<ActiveLog> = Vec::new();
...
//         all_possibilities.push(ActiveLog::new(
//             net_deviation,
...
//     // Try inserting at each position - but we only want the FIRST successful insertion
//     for index in batch_index..schedule.batches.len() {
//         let current_prev_completion = if index == 0 {
//             0
//         } else if index == batch_index {
...
//                 }
//                 Ordering::Greater => {
//                     // Calculate deviation for inserting job in current batch
//                     let (current_deviation, completion) =
//                         compute_current_deviation(&job_list, job, current_prev_completion);
                    
//                     // Get all possibilities for the newly displaced job
//                     let recursive_possibilities = get_active_logs_for_displaced_job(
//                         index + 1,
//                         schedule,
//                         &lp_job,
//                         completion,
...
//                 },
//             ));
//             return all_possibilities; // Return this direct insertion
//         }
//     }
//       // If no insertion worked, create new batch at the end
//     let release_date = job.release_date.max(prev_completion);
//     let completion = release_date + job.processing_time;
//     let net_deviation = job.due_date as i32 - completion as i32;
